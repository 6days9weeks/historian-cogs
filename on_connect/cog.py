...
class OnConnect(commands.Cog):
    """ Series of events for the shards thingy ig"""

    def __init__(self, bot):
        self.bot = bot
        self.config = Config.get_conf(self, identifier=0xA203A2A, force_registration=True)
        default_global = {
            "event_channel": None,
            "type": 1,
        }
    ...
    async def build_cache(self):
        ...
        if await self.config.type() == 0 and await self.config.event_channel() != None:
            channel = await self.chan_get_or_fetch(int(await self.config.event_channel()))
    ...
    async def send_event(self, embed: discord.Embed):
        ...
        if self.type == 0 and self.webhook != None:
            await self.webhook.send(
                embed=embed, username=self.bot.user.name, avatar_url=self.bot.user.avatar_url
            )
        else:
    ...
    async def on_shard_ready(self, shard_id: int):
        ...
        embed.description = f"\N{LARGE GREEN CIRCLE} Shard `{shard_id}` is now ready."
        await self.send_event(embed)
    ...
    async def on_ready(self):
        ...
        embed = discord.Embed(
            color=0x81E0A9,
            title=f"{self.bot.user.name} - is now online",
            timestamp=dt.utcnow(),
        )
        embed.description = f"\N{LARGE GREEN CIRCLE} I am ready."
        await self.send_event(embed)
    ...
    async def on_shard_disconnect(self, shard_id: int):
        ...
        embed = discord.Embed(
            color=0xE74C3C,
            title=f"{self.bot.user.name} - Shard Disconnect",
            timestamp=dt.utcnow(),
        )
        embed.description = (
            f"\N{NO ENTRY}\N{VARIATION SELECTOR-16} Shard `{shard_id}` just disconnected."
        )
        await self.send_event(embed)
    ...
    async def on_shard_resumed(self, shard_id: int):
        ...
        embed = discord.Embed(
            color=0x81E0A9,
            title=f"{self.bot.user.name} - Shard Resumed",
            timestamp=dt.utcnow(),
        )
        embed.description = (
            f"\N{BALLOT BOX WITH CHECK}\N{VARIATION SELECTOR-16} Shard `{shard_id}` just resumed."
        )
        await self.send_event(embed)
    ...
    async def type(self, ctx: commands.Context, type: int):
        ...
        if type == 0:
            channel = self.bot.get_channel(int(await self.config.event_channel()))
